...
The script requires you to download download a tika server file (both tika-server.jar and tika-server.jar.md5, which can be found her https://repo1.maven.org/maven2/org/apache/tika/tika-server-standard/ - and include within the clone folder, it also needs Python 3 and Java installed to run. Your python will need the module tika installed to run - https://github.com/chrismattmann/tika-python. You will also need to install pandas. Pip install pandas and pip install tika.
...
If script fails to run it may be because it has failed to launch tika jar, open up a terminal window and run the tika-server, paste the following after the 'U:\>': java -jar [file path of the .jar Apache Tika file] and try again.
